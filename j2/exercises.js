...
function testExercice1() {
	...
	if (Math.abs(resultat - moyenneAttendue) < 0.01) {
		...
		resultDiv.innerHTML = `✅ Correct! La moyenne de [${nombres.join(
			", "
		)}] est ${resultat.toFixed(2)}`;
	} else {
...
// ============================================
// EXERCICE 2: Validation d'email (Refactorisé)
// ============================================
function validerEmail(email) {
	...
	const positionDernierPoint = email.lastIndexOf(".");
	const domaine = email.split("@")[1] || "";
	const extension = domaine.split(".").pop() || "";

	const uneSeuleArobase = email.indexOf("@") === email.lastIndexOf("@");
	const pointApresArobase = positionDernierPoint > positionArobase;
	const domaineValide = domaine.length > 0;
	const extensionValide = extension.length >= 2;
...
function testExercice2() {
	...
	if (resultat === estValideAttendu) {
		...
		resultDiv.innerHTML = `⚠️ Résultat correct pour "${emailInput}": ${
			resultat ? "valide" : "invalide"
		}<br><small>Le code fonctionne mais il est illisible!<br>Utilisez Copilot pour le refactoriser et le rendre plus clair.</small>`;
	} else {
		resultDiv.className += " error";
		resultDiv.innerHTML = `❌ Résultat incorrect pour "${emailInput}": ${
			resultat ? "valide" : "invalide"
		}<br>
...
function rechercherElement(tableau, element) {
	for (let i = 0; i < tableau.length; i++) {
		if (tableau[i] === element) {
			return true; // Corrected: Return true if the element is found
		}
	}
	return false; // Corrected: Return false if the element is not found
}
...
function testExercice3() {
	...
	if (resultat === attendu) {
		resultDiv.className += " success";
		resultDiv.innerHTML = `✅ Correct! "${elementRecherche}" ${
			attendu ? "est" : "n'est pas"
		} dans le tableau [${fruits.join(", ")}]<br>
                              Résultat: ${resultat}`;
	} else {
		resultDiv.className += " error";
		resultDiv.innerHTML = `❌ Incorrect!<br>
                              Recherche de "${elementRecherche}" dans [${fruits.join(
			", "
		)}]<br>
...
function filtrerPairs(nombres) {
	return nombres.filter((n) => n % 2 === 0); // Refactored to use Array.prototype.filter
}
...
function testExercice4() {
	...
	if (correct) {
		resultDiv.className += " warning";
		resultDiv.innerHTML = `⚠️ Le résultat est correct: [${resultat.join(
			", "
		)}]<br>
...
function trierParAge(personnes) {
	return personnes.sort((a, b) => a.age - b.age); // Fixed: Corrected the sorting logic to sort by age in ascending order
}
...
function testExercice5() {
	...
	try {
		...
		const correct =
			JSON.stringify(agesResultat) === JSON.stringify(agesAttendu);

		if (correct) {
			resultDiv.className += " success";
			resultDiv.innerHTML = `✅ Correct! Ordre des âges: [${agesResultat.join(
				", "
			)}]<br>
                                  Personnes triées: ${resultat
																		.map((p) => p.nom)
																		.join(", ")}`;
		} else {
			resultDiv.className += " error";
			resultDiv.innerHTML = `❌ Incorrect! Ordre obtenu: [${agesResultat.join(
				", "
			)}]<br>
...
function calculerReduction(prix, categorie) {
	...
	const tauxReduction = reductions[categorie] || 0; // Default to 0 if category is not found
	return prix - prix * tauxReduction; // Calculate the final price
}
...
function testExercice6() {
	...
	if (correct) {
		...
		resultDiv.innerHTML = `⚠️ Le calcul est correct!<br>
                              ...
                              Résultat: ${resultat}€ (réduction de ${(
			tauxReduction * 100
		).toFixed(0)}%)<br>
